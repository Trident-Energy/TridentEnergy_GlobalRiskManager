...
interface Props {
  ...
  // New props for Selection & Consolidation
...
const RiskList: React.FC<Props> = ({ 
  ...
  const toggleRowExpansion = (e: React.MouseEvent, riskId: string) => {
    e.stopPropagation();
  ...
  const renderRow = (risk: Risk, depth: number = 0) => {
    ...
    return (
      ...
      >
           ...
           <td className="px-4 py-3 w-14 text-center">
             <div onClick={(e) => e.stopPropagation()} className="flex justify-center">
               <input 
                 type="checkbox" 
                 checked={isSelected} 
                 onChange={() => onToggleSelection && onToggleSelection(risk.id)}
        ...
        {/* Expansion Chevron (Consolidated View) */}
        {isConsolidatedView && (
        ...
        <table className={`w-full text-left border-collapse ${tableMinWidth}`}>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {paginatedRisks.length === 0 ? (
              <tr>
                <td colSpan={20} className="px-6 py-8 text-center text-slate-500 dark:text-slate-400">
                  No risks found matching your filters.
                </td>
              </tr>
            ) : (
              paginatedRisks.map(risk => renderRow(risk))
            )}
          </tbody>
        </table>
      </div>
