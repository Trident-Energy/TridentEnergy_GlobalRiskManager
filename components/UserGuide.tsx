...
const UserGuide: React.FC = () => {
  ...
  // Helper to get cell color for the static matrix
  const getCellColor = (score: number) => {
    if (score >= 15) return 'bg-[#C0392B] text-white'; // Red
    if (score >= 5) return 'bg-[#F1C40F] text-slate-900'; // Yellow
    return 'bg-[#6FCF97] text-white'; // Green
  };
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                   ...
                   <p className="text-sm text-slate-600 dark:text-slate-400 mb-4 leading-relaxed">
                      The level of risk that exists before any controls or mitigation measures are applied. It represents the natural exposure associated with an activity, process, or environment simply due to its nature.
                   </p>
                ...
                <div>
                   ...
                   <p className="text-sm text-slate-600 dark:text-slate-400 mb-4 leading-relaxed">
                      The level of risk that remains after controls or risk treatments have been implemented. It reflects the effectiveness of those controls in reducing inherent risk.
                   </p>
                </div>
             </div>
          ...
          <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-8 shadow-sm animate-fade-in">
             ...
             <div className="space-y-6">
                <p className="text-slate-600 dark:text-slate-300 mb-4">
                   Risks are visualized on a 5x5 matrix where the score determines urgency.
                </p>
                
                {/* Embedded Visual Matrix Table */}
                <div className="overflow-x-auto mb-8">
                   <div className="inline-block min-w-full">
                      <div className="flex">
                         {/* Y-Axis Label */}
                         <div className="flex items-center justify-center w-8 bg-slate-100 dark:bg-slate-800 rounded-l-lg border-y border-l border-slate-200 dark:border-slate-700">
                            <span className="-rotate-90 whitespace-nowrap font-bold text-slate-500 text-xs uppercase tracking-widest">Likelihood</span>
                         </div>
                         ...
                         <div className="flex-1">
                            <table className="w-full text-center border-collapse">
                               <thead>
                                  <tr>
                                     <th className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 w-32"></th>
                                     <th className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300">1 - Negligible</th>
                                     <th className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300">2 - Marginal</th>
                                     <th className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300">3 - Average</th>
                                     <th className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300">4 - Severe</th>
                                     <th className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300">5 - Catastrophic</th>
                                  </tr>
                               </thead>
                               <tbody>
                                  {/* Row 5 */}
                                  <tr>
                                     <td className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300 text-left">5 - Frequent</td>
                                     {[5, 10, 15, 20, 25].map(score => (
                                        <td key={score} className={`p-3 border border-white dark:border-slate-900 font-bold text-lg ${getCellColor(score)}`}>
                                           {score}
                                        </td>
                                     ))}
                                  </tr>
                                  {/* Row 4 */}
                                  <tr>
                                     <td className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300 text-left">4 - Likely</td>
                                     {[4, 8, 12, 16, 20].map(score => (
                                        <td key={score} className={`p-3 border border-white dark:border-slate-900 font-bold text-lg ${getCellColor(score)}`}>
                                           {score}
                                        </td>
                                     ))}
                                  ...
                                  {/* Row 3 */}
                                  <tr>
                                     <td className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300 text-left">3 - Unlikely</td>
                                     {[3, 6, 9, 12, 15].map(score => (
                                        <td key={score} className={`p-3 border border-white dark:border-slate-900 font-bold text-lg ${getCellColor(score)}`}>
                                           {score}
                                        </td>
                                     ))}
                                  ...
                                  {/* Row 2 */}
                                  <tr>
                                     <td className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300 text-left">2 - Remote</td>
                                  ...
                                  {/* Row 1 */}
                                  <tr>
                                     <td className="p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-bold text-xs text-slate-600 dark:text-slate-300 text-left">1 - Extremely Remote</td>
                                     {[1, 2, 3, 4, 5].map(score => (
                                        <td key={score} className={`p-3 border border-white dark:border-slate-900 font-bold text-lg ${getCellColor(score)}`}>
                                           {score}
                                        </td>
                                  ...
                                  <tr>
                                     ...
                                     <td colSpan={5} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-b-lg border-x border-b border-slate-200 dark:border-slate-700 text-center font-bold text-slate-500 text-xs uppercase tracking-widest">
                                        Impact
                                     </td>
                                  </tr>
                               </tbody>
          ...
          <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-8 shadow-sm animate-fade-in">
             ...
             <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-xl border border-blue-100 dark:border-blue-800 mb-8">
                <p className="text-slate-800 dark:text-slate-200 font-medium mb-3">
                   A control is an action, process, or measure that reduces either:
                </p>
                <ul className="list-disc list-inside text-sm text-slate-700 dark:text-slate-300 space-y-2 mb-6 ml-2">
                   <li>The chance of the risk happening (<strong>likelihood</strong>), or</li>
                   <li>The damage it would cause if it did happen (<strong>impact</strong>).</li>
                </ul>

                <p className="text-slate-800 dark:text-slate-200 font-medium mb-3">
                   Controls can be:
                </p>
                <ul className="list-disc list-inside text-sm text-slate-700 dark:text-slate-300 space-y-2 ml-2">
                   <li><strong>Preventive</strong> – stop the risk from occurring.</li>
                   <li><strong>Detective</strong> – identify the risk early if it happens.</li>
                   <li><strong>Corrective</strong> – reduce the impact once it has happened.</li>
                </ul>
             </div>

             <h4 className="font-bold text-slate-800 dark:text-white mb-4">Rating Control Effectiveness</h4>
