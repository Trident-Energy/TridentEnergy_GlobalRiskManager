...
const App = () => {
  ...
  // Default visible columns: all except 'id' and 'parentRiskTitle'
  const [visibleColumns, setVisibleColumns] = useState<string[]>(
    AVAILABLE_COLUMNS.map(c => c.key).filter(key => key !== 'id' && key !== 'parentRiskTitle')
  );
  ...
  const handleConsolidateRisks = () => {
     ...
     const selected = Array.from(selectedRiskIds) as string[];
  ...
  const consolidatedRisksList = useMemo(() => {
     ...
     return dashboardRisks.filter(r => {
        // Prevent Double Counting: If risk is inside a consolidated group, hide it from top-level list
        if (groupedRiskIds.has(r.id)) return false;
  ...
  const myEscalationsCount = useMemo(() => {
    return dashboardRisks.filter(r => !r.isConsolidatedGroup && r.escalations?.some(e => e.userId === currentUser.id)).length;
        ...
        <div className="w-full px-6 py-6">
                             ...
                             {AVAILABLE_COLUMNS.map(col => (
                               ...
                               >
                                  ...
                                  <span className="font-bold">
                                    {col.label || '(Icon Column)'} {col.mandatory && <span className="text-[10px] text-slate-400 ml-1">(Required)</span>}
                                  </span>
          ...
          {currentView === 'dashboard' && (
            <div className="space-y-6 animate-fade-in w-full">
              {/* KPIs & Quick Filters */}
              <RiskDashboard 
                risks={dashboardRisks} // Pass full filtered list (Component handles exclusion of consolidated groups for KPIs)
              ...
              <RiskList 
                risks={listRisks}
                allRisksSource={risks} // Pass full list for parent lookup
                actions={actions}
                comments={comments}
                currentUser={currentUser}
                onSelectRisk={(r) => {
             ...
             <div className="space-y-6 animate-fade-in w-full">
                <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                   <div className="flex items-center gap-4 mb-4">
                      ...
                      <div>
                         <h2 className="text-xl font-bold text-slate-800 dark:text-white">Consolidated Dashboard</h2>
         ...
         <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-8 max-w-2xl w-full border-t-4 border-purple-500">
               ...
               <p className="text-slate-600 dark:text-slate-300 mb-6">
                  You are grouping <strong>{selectedRiskIds.size}</strong> items into a new Parent Risk.
               </p>
